...
echo "   device=${DEVICE} config=${CONFIG} crystal_steps=${CRYSTAL_STEPS}"
...
python scripts/train.py \
  ...
  --checkpoint "${WITHOUT_CKPT}" --device "${DEVICE}" \
...
python scripts/train.py \
  --config "${CONFIG}" \
  --train-data "${TRAIN}" --val-data "${VAL}" --test-data "${TEST}" \
  --checkpoint "${WITH_CKPT}" --device "${DEVICE}" \
...
python scripts/analysis/export_checkpoint_predictions.py \
  ...
  --output "${WITHOUT_PRED}" --model-type tgnn --device "${DEVICE}"
python scripts/analysis/export_checkpoint_predictions.py \
  --checkpoint "${WITH_CKPT}" --data "${TEST}" \
  --output "${WITH_PRED}" --model-type tgnn --device "${DEVICE}"
