...
PKA_VAR = {"low_F": 9.612, "high_F": 8.235}
...
def main():
    ...
    for strat, name in [("low_F", "ortho (misspecified)"), ("high_F", "meta/para (well-specified)")]:
        ...
        print(f"   B_closure(point)={bclos:.3f} CI[{bclos_lo:.3f},{bclos_hi:.3f}]  "
              f"B_closure_norm={bcn:.3f} CI[{bcn_lo:.3f},{bcn_hi:.3f}]")
        if strat in S_NORM_MEAS:
            req = (S_NORM_MEAS[strat] - SYN_LAW["intercept"]) / SYN_LAW["slope"]
            inside = bcn_lo <= req <= bcn_hi
            print(f"   frontier law needs x={req:.3f}; point x={bcn:.3f} CI[{bcn_lo:.3f},{bcn_hi:.3f}]")
            print(f"   => law {'CONSISTENT (needed x in CI)' if inside else 'REFUTED (needed x outside CI)'}")
