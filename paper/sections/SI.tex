...
at evaluation.
\paragraph{A reproducibility caution for \texttt{cCOSMO} (COSMO-SAC-2002 on typed profiles).}
We flag a silent-failure mode of the NIST reference implementation that a comparison of
closures can easily trip over, and that produced the mislabelled $\mathrm{MSE}\ 1.26$ we
withdraw in \S\ref{sec:measure}. The 2002 model (\texttt{COSMO1}) expects a single untyped
$\sigma$-profile. When it is instead handed a typed three-profile (Hsieh/2010, \texttt{sigma3})
database --- the natural thing to do when both a 2002 and a 2010 arm are to be run off one
profile source --- it reads the nonpolar channel only, discards the donor and acceptor
profiles, and keeps the total cavity area as prefactor. It returns a number without error.
That crippled 2002 scores $\mathrm{MSE}\ 1.263$ on our matched corner, whereas the fair full
2002 --- the same profile fed either to our differentiable layer or to \texttt{cCOSMO}
reformatted into the three-profile layout --- scores $1.757/1.758$. Anyone contrasting 2002
against 2010 by calling \texttt{COSMO1} and \texttt{COSMO3} on a shared \texttt{sigma3} base is
therefore comparing a two-channels-deleted 2002 to a full 2010 and will overstate the fidelity
gain; the fair 2002 requires the native single-profile (Mullins) base or the reformatted
layout.
...
drops the closure, so the gap to the physics path is the asymptotic physics penalty
...
\end{proposition}
\begin{proof}[Proof of Proposition~\ref{prop:tax} (sign of the asymptotic physics penalty)]
...
cited, not redistributed). The Bemis--Murcko scaffold split follows the main text; because the seeded
