...
$n{=}60$ set (\texttt{run\_b\_insuff\_robustness.py}). \emph{(i)~Estimator sensitivity.} The
$\Binsuf$ upper bounds are $0.40$ ($k$-NN), $0.56$ (binning/LOTV), $0.62$ (random forest), and
$0.88$ (ridge)---computed with this section's estimator settings, so they differ slightly from
Table~\ref{tab:decomp}, a hyperparameter effect, not a convention one ($\Binsuf$ is
convention-independent, not estimator-independent). The spread is a bias--variance ordering of the
same target $\Ex[\Var(m\mid z)]$: the local $k$-NN estimate is the least smoothed and sits lowest,
while the global ridge smoother is the most regularized and sits highest. Because a \emph{smaller}
$\Binsuf^{\mathrm{up}}$ is the \emph{less} conservative input to
$\Bclos\ge\mathrm{MSE}-\Binsuf^{\mathrm{up}}$, we do not lean on the $k$-NN value despite its being
the smallest---it is the most optimistic, not the most trustworthy. With
$\mathrm{MSE}_{\mathrm{res}}=1.47$ the verdict $\Bclos>\Binsuf$ holds under the three lower
estimators ($k$-NN, binning/LOTV, random forest) and fails only under the most conservative ridge
estimator---non-certifying there, not refuting, and consistent with the clustered-bootstrap
``likely but not certified'' reading below. \emph{(ii)~Leave-one-solvent-out.} Dropping each of the $17$ solvents in turn, the
...
(\texttt{run\_b\_insuff\_regime.py}) tests whether closure-dominance is a corner artifact. It is
not: the verdict $\Bclos>\Binsuf$ (tightest of the binning/RF/$k$-NN upper bounds) holds in every
stratum and \emph{strengthens} toward higher $\gamma$ (Table~\ref{tab:regime})---in the highest
stratum ($\overline{\lngi}=2.9$, approaching the non-ideal regime) $\Bclos\ge3.0$ dominates
$\Binsuf\le0.3$ by an order of magnitude. We caution that the high-$\gamma$ strata are small
...
it is the large ($\sim\!10\times$) separations rather than the precise values that carry the
...
residual-only convention; $\Binsuf^{\mathrm{up}}$ is the tightest of the binning / RF / $k$-NN
upper bounds). The separation holds in every stratum and grows with $\gamma$.}
\label{tab:regime}
\begin{tabular}{lrrrr}
\toprule
Stratum & $n$ & $\overline{\lngi}$ & $\Binsuf\le$ & $\Bclos\ge$ \\
\midrule
all                         & $60$ & $+0.75$ & $0.40$ & $1.07$ \\
lower $\gamma$ ($m\le$ med.) & $30$ & $-0.40$ & $0.07$ & $0.95$ \\
higher $\gamma$ ($m>$ med.)  & $30$ & $+1.90$ & $0.28$ & $1.65$ \\
high $\gamma$ ($m>1.5$)      & $17$ & $+2.94$ & $0.32$ & $3.04$ \\
\bottomrule
\end{tabular}
\end{table}
