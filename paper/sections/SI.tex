...
at evaluation.
\paragraph{A usage pitfall in \texttt{cCOSMO} (\texttt{COSMO1} on typed profiles), and a correction
to our own earlier reading.} The $\mathrm{MSE}\ 1.26$ we previously reported for the 2002 kernel and
withdraw in \S\ref{sec:measure} was a usage error on our part, worth documenting only because the
library gives no warning. The reference implementation~\cite{bell2020cosmosac} pairs its 2002 model
...
nonpolar-only read is neither that sum nor a documented mode. The crippled arm scores
$\mathrm{MSE}\ 1.263$ on our matched corner, whereas the fair full 2002 --- the same molecule fed
either to our differentiable layer or to \texttt{cCOSMO} on its native single-profile base --- scores
$1.757/1.758$. We report this as an issue for the library maintainers (a silently-wrong return where
an error would be safer), not as a defect of any published benchmark.
