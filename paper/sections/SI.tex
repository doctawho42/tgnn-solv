...
qualitative reading. The $m>\overline{m}$ stratum separates
...
\begin{enumerate}
  ...
  the headline claim, together with the combinatorial Tier~2 fact.
...
We retrained the direct state-of-the-art model (FastSolv \cite{osanloo2024}) and the
physics-informed state-of-the-art model (SolProp \cite{vermeire2022solprop}) on our corpus and evaluated them on a
...
scaffold-split DirectGNN ($1.70$) outperforms them, and a like-for-like scaffold evaluation of the external models is left to
...
profile availability. Second-domain pKa: the OPERA experimental compilation
...
\bottomrule
\end{tabular}
\caption{Study 1a: crystal-term decodability per seed, with the permutation null, the drop-one-solute jackknife and the leverage carried by the worst solutes.}
\end{table*}
...
\bottomrule
\end{tabular}
\caption{Study 2: the model's temperature response against the empirical slope and against $-\Delta H_{\text{fus}}/R$, with the curvature of both in $1/T$.}
\end{table*}
...
\bottomrule
\end{tabular}
\caption{Study 4: share of representation variance per organising factor, as an excess over a permutation null matched to the level at which that factor varies.}
\end{table*}
