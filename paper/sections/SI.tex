...
convention-independent, not estimator-independent). Since the true $\Binsuf$ lies below the
smallest valid upper bound and
$\mathrm{MSE}_{\mathrm{res}}=1.47$, the verdict $\Bclos\ge\mathrm{MSE}-\Binsuf^{\mathrm{up}}>\Binsuf$
certifies under the three tightest estimators (the ridge bound is loose---non-certifying, not
refuting). \emph{(ii)~Leave-one-solvent-out.} Dropping each of the $17$ solvents in turn, the
