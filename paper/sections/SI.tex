...
solubility \emph{paradox} (reference minus learned $\lnx$ MAE), the symptom, not the profile drift
directly: on the full test set it is $+1.13$ (95\% CI $[0.88,1.38]$) under the minimal objective,
\emph{larger} than the $+0.30$ ($[0.20,0.39]$) under the full one. The compensation is thus intrinsic and
strengthens once the activity-grounding auxiliaries, the only pressure toward physics, are removed. This
is \emph{consistent with} the compensating surrogate being intrinsic to end-to-end training through a
...
(seed $42$; \texttt{scripts/analysis/run\_loss\_ablation\_analysis.py}).
...
reference profiles reaches a floor of MSE $\approx0.40$ (our least-conservative, $k$-NN-adjacent
$\Binsuf$ estimate; the leakage-immune LOTV upper bound is $0.62$, \S\ref{sec:measure}), so the
closure's excess over the leakage-immune floor is at least ${\approx}2.4\times$ and is $\Bclos$.}
\label{fig:parity}
\end{figure}
