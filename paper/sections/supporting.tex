...
still: the gap becomes $+1.09$ $[+0.3,+2.2]$ pooled---one decimal, because that is the precision
$4000$ cluster-bootstrap draws pin for this one interval, whose second decimal moves with the
draw---$+1.63/{+}0.49/{+}1.14$ by seed, each interval excluding zero, though seed $44$ only just:
its lower bound is $+0.009$, and an independent recomputation places it on the other side, as the
boundary cell of Table~\ref{tab:ranking} does. Seed $43$'s bound, $+0.013$, is close enough to
change side occasionally; seed $42$'s, $+0.221$, is not. Its point estimate is larger than the raw
...
nDCG@3 are defined there and score all $823$.
