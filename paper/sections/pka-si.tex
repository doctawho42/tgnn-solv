...
where through-conjugation ($\sigma^-$) breaks it---$\Bclos$ observed in practice.
...
makes $\Gamma$ an indicator rather than a certificate. The oracle-$\sigma$ pipeline
...
few others---inconsistent with a neutral-scaffold Hammett assignment, and they degrade the clean pole
...
\caption{pKa through a Hammett LFER, on a controlled construction with known ground-truth
channels. \emph{Left:} closure fidelity varied with a sufficient intermediate; grounding gap
$\Gamma$, closure bias $\Bclos$, and the assumption-light Jensen bound. \emph{Right:} an
ortho/steric insufficiency channel varied with a \emph{well-specified} closure; $\Bclos=0$
throughout while $\Gamma>0$ and grows.}
\label{fig:pka}
\end{figure}
