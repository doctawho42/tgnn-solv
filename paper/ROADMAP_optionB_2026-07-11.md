...
- [x] **SI fully populated** — S1 constants/loss, S2 proofs, S3 per-arm table + hedges (moved from body), S5 external baselines, S7 mechanism (corrupted-twin/45K-crystal/ΔCp/fusion-scarcity/NRTL-collapse) + temperature table, S9 provenance/compute-ledger.
- [x] **Data-efficiency figure** (`plot_data_efficiency.py` → `fig_data_efficiency`).
- [x] **Readability**: abstract rewrite (P0), §measure hedges → SI, §paradox de-densify (P1). _(Optional remaining polish: P3 lemma glosses, P4 tabulate ΔT/crystal deltas — minor.)_
- **CONTENT-COMPLETE.** Remaining = user-gated only: 4 `\pending` (co-authors, repo URL, license, Zenodo DOI) + optional architecture-figure redraw (current Fig.1 is COSMO-only per audit). Final consistency check running.
