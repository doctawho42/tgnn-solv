...
| `scripts/evaluation/run_directgnn_error_structure_diagnostics.py` | Structural error diagnostics for the maintained DirectGNN scaffold checkpoint | Stable utility | Computes train/val/test gap, pair-level error distribution, chemistry buckets, and nearest-neighbor error summaries |
| `scripts/evaluation/run_prediction_error_slices.py` | Prediction-only error slicing for existing model CSVs | Stable utility | Builds aligned pair, chemistry, solvent-type, chemistry-by-solvent, halogenated-aromatic, paired-delta, and nearest-neighbor summaries without retraining |
| `scripts/evaluation/plot_prediction_error_slices.py` | Plot prediction error slice bundles | Stable utility | Generates PNG/PDF presentation figures from `results/prediction_error_slices/` and copies them into `presentation/figures/generated/` by default |
| `scripts/evaluation/run_temperature_extrapolation_baselines.py` | CPU-first temperature extrapolation baselines | Stable utility | Builds a same-pair low-T/high-T split, writes reusable neural split CSVs, evaluates pair-level Van't Hoff/linear/constant and RF(Morgan+T) baselines, and generates presentation figures |
| `scripts/evaluation/run_temperature_interpolation_baselines.py` | CPU-first in-pair temperature interpolation baselines | Stable utility | Keeps pair temperature endpoints in train, holds out interior temperatures, writes reusable neural split CSVs, evaluates pair-level curve baselines and RF(Morgan+T), and generates presentation figures |
| `scripts/data/build_vant_hoff_anchor_split.py` | Build pseudo high-T Van't Hoff anchor rows | Research utility | Fits pairwise `ln(x2)` vs `1/T` on train-low rows and appends `vh_anchor` pseudo rows for temperature-extrapolation objective ablations |
...
| `scripts/data/extract_idac_from_thermoml.py` | Build `idac.csv` from NIST ThermoML JSON records | Stable utility | Enables real `gamma_inf` supervision; reads DOI files, local JSON archives, official NIST issue pages, and NIST issue-index JSON via `--expand-journal-issues`; writes DOI/audit sidecars for large crawls |
| `scripts/data/extract_crystal_from_thermoml.py` | Build ThermoML `T_m` / `dH_fus` crystal artifacts | Stable utility | Extracts pure-component crystal measurements from local or fetched ThermoML JSON, preserves DOI/method/phase provenance, and writes both raw and aggregated crystal CSVs |
| `scripts/data/extract_activity_from_thermoml.py` | Build finite-composition ThermoML activity artifacts | Stable utility | Extracts finite-composition `Activity coefficient`, `(Relative) activity`, and excess-mixing enthalpy rows, excludes the separate IDAC regime, writes exact-state aggregates, overlap audits, and temperature-span summaries |
| `scripts/data/collect_targeted_thermoml_coverage.py` | Audit exact ThermoML coverage for maintained SLE pairs | Stable utility | Scans local or fetched ThermoML JSON for exact binary pair overlap, expands unordered ThermoML matches to directed SLE pairs, writes measurement-level exact-state harvests, and emits both label-level and measurement-backed candidate gap lists for targeted collection |
| `scripts/data/build_unifac_finite_activity_coverage.py` | Build finite-composition Modified-UNIFAC pseudo-coverage for SLE pairs | Stable utility | Evaluates finite-composition `ln(gamma)` on exact SLE pairs over a composition grid, focuses by default on the current measurement-backed ThermoML gap list, and reports pair/state coverage plus missing-group reasons |
| `scripts/data/build_open_crystal_artifact.py` | Consolidate open crystal sources with explicit priority | Stable utility | Merges curated crystal values, ThermoML aggregates, and Bradley melting points into one source-aware solute artifact, then audits gain versus canonical processed splits |
| `scripts/data/attach_idac_aux_to_fixed_splits.py` | Attach expanded IDAC to fixed split CSVs | Stable utility | Preserves canonical supervised train/val/test rows and appends new `gamma_inf` aux rows to train only for fair IDAC ablations |
| `scripts/analysis/audit_idac_expansion.py` | Audit and consolidate expanded IDAC data | Stable utility | Writes exact-deduplicated raw IDAC, aggregated training IDAC, conflict reports, coverage summaries, and figures |
| `scripts/analysis/audit_dcp_correction.py` | Estimate omitted SLE `dCp` correction scale | Stable utility | Uses maintained Joback/GC priors to quantify correction magnitude and highlight unsafe uncalibrated-prior regimes |
| `scripts/analysis/audit_fusion_supervision.py` | Audit `T_m` / `dH_fus` / derived `dS_fus` supervision | Stable utility | Quantifies direct crystal-label coverage, Walden interval violations, and descriptor correlations for entropy-coupled fusion ablations |
| `scripts/analysis/summarize_physics_supervision_readiness.py` | Summarize physics-supervision readiness | Stable utility | Builds a combined JSON/Markdown report from IDAC, fixed-split aux, dCp, unit-conversion, and error-slice diagnostics |
| `scripts/analysis/scan_thermoml_property_inventory.py` | Inventory cached ThermoML properties | Stable utility | Counts property/method/variable/constraint labels to scope GE, VLE, heat-capacity, and other auxiliary extractors |
| `scripts/analysis/audit_water_small_molecule_graphs.py` | Audit water and small-solvent graph topology | Stable utility | Quantifies water/small-solvent corpus coverage, compares legacy self-loop vs explicit-H graphs, and reports current water/small-solvent error slices |
| `scripts/analysis/run_structural_extrapolation_diagnosis.py` | Scaffold structural-extrapolation diagnosis | Research utility | Consumes aligned prediction slices and writes model-delta, novelty-bin, target-bin, chemistry-class, solvent-type, rescue, and regression summaries |
